...
def evaluate():
    ...
    for idx, img_path in enumerate(tqdm(candidates, desc="Evaluating")):
        ...
        if np.sum(true_mask) < 200: continue # Skip very tiny tumors
        ...
        for t_val in NOISE_LEVELS:
            ...
            img_flipped_v = torch.flip(img_tensor, [2])  # vertical flip
            diff_flipped_v, _ = process_one_pass(model, diffusion, img_flipped_v, t_val)
            diff_3 = np.flip(diff_flipped_v, axis=0).copy()
            
            # 3. Average Predictions
            diff = (diff_1 + diff_2 + diff_3) / 3.0
            ...
            for p in possible_percentiles:
                ...
                if dice > best_dice:
                    best_dice  = dice
                    best_auroc = auroc
                    best_precision = precision_score(true_mask.flatten(), pred_mask.flatten(), zero_division=0)
                    best_recall    = recall_score(true_mask.flatten(),    pred_mask.flatten(), zero_division=0)
                    best_f1        = f1_score(true_mask.flatten(),        pred_mask.flatten(), zero_division=0)
                    ...
                    # Original Background + Healed Tumor Area
   ...
   # FINAL SUMMARY — HONEST FULL DATASET REPORTING
    if len(valid_results) > 0:

        # ── All samples (honest average) ─────────────────────────────────
        all_dice  = [x["dice"]  for x in valid_results]
        all_auroc = [x["auroc"] for x in valid_results]
        all_ips   = [x["ips"]   for x in valid_results]

        avg_dice_all  = np.mean(all_dice)
        avg_auroc_all = np.mean(all_auroc)
        avg_ips_all   = np.mean(all_ips)
        std_dice_all  = np.std(all_dice)

        print("\n" + "="*65)
        print("  FULL DATASET RESULTS (ALL SAMPLES — HONEST AVERAGE)")
        print("="*65)
        print(f"  Total valid samples evaluated : {len(valid_results)}")
        median_dice = np.median(all_dice)
        iqr_dice    = np.percentile(all_dice, 75) - np.percentile(all_dice, 25)
        print(f"  Mean DICE     : {avg_dice_all:.4f}  ± {std_dice_all:.4f}")
        print(f"  Median DICE   : {median_dice:.4f}  (IQR: {iqr_dice:.4f})")
        std_auroc_all = np.std(all_auroc)
        std_ips_all   = np.std(all_ips)
        print(f"  Average AUROC                 : {avg_auroc_all:.4f}  ± {std_auroc_all:.4f}")
        print(f"  Average IPS                   : {avg_ips_all:.4f}  ± {std_ips_all:.4f}")
        print("="*65)

        dices = sorted(all_dice)
        print("\n  DICE percentiles (all samples):")
        for p in [10, 25, 50, 75, 90]:
            print(f"    P{p:2d} : {np.percentile(dices, p):.4f}")

        avg_precision = np.mean([x["precision"] for x in valid_results])
        avg_recall    = np.mean([x["recall"]    for x in valid_results])
        avg_f1        = np.mean([x["f1"]        for x in valid_results])

        print(f"  Average Precision             : {avg_precision:.4f}")
        print(f"  Average Recall                : {avg_recall:.4f}")
        print(f"  Average F1-Score              : {avg_f1:.4f}")
        print("="*65)
        print(f"\n  Visuals saved to: {OUTPUT_VISUALS}")
